...
// ===== Config =====
const SCHEMA_SRC_DIR = path.resolve(process.cwd(), "src/main/resources/qip-model");
...
function writeSchema(filePath: string, data: any) {
    if (filePath.endsWith(".yaml") || filePath.endsWith(".yml")) fs.writeFileSync(filePath, yaml.dump(data), "utf8");
...
function rewriteRefsAndIds(obj: any, currentFile: string): any {
    if (Array.isArray(obj)) return obj.map(item => rewriteRefsAndIds(item, currentFile));
            ...
            if ((key === "$ref" || key === "$id") && typeof value === "string") {
                ...
                if (value.startsWith(REF_PREFIX)) {
                    ...
                    if (!fs.existsSync(targetAbsolute)) {
                        console.warn(`Missing target for ${key}: ${value} → ${targetAbsolute}`);
                    }
                }
                newObj[key] = newVal;
            } else {
